 ...
 *					    the intended target core is on
...
static int gic_set_type(struct irq_data *d, unsigned int type)
{
	...
	if (type != IRQ_TYPE_LEVEL_HIGH && type != IRQ_TYPE_EDGE_RISING) {
		 ...
		 * to allow this to pass through, TBD needs a fix upstream.
		 */
		if (!((gicirq >= 16 && gicirq <= 32) &&
...
static int _gic_set_affinity(struct irq_data *d, unsigned int cpu,
			     bool do_clear)
{
			    ...
			    bool force)
{
	...
	if (on_same_cluster(mpidr_to_hwcpu(cpu_logical_map(cpu)), pcpu)) {
		...
		ret = exec_remote_set_affinity(false, mpidr_to_hwcpu(cpu_logical_map(cpu)),
					       d, mask_val, force);
	 ...
	 * on the old cluster.
	 */
	if (!on_same_cluster(mpidr_to_hwcpu(cpu_logical_map(cpu)),
			     irq_cpuid[irqid]) && irqid != IRQ_PMU) {
		/*

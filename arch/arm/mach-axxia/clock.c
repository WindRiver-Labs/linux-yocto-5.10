...
static void axxia_register_clks(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(dt_clks); ++i) {
		...
		clk[c->id] = clk_register_fixed_rate(NULL, c->name,
						     NULL, CLK_IS_ROOT, freq);
	}

	/* APB clock dummy */
	clk[clk_apb] = clk_register_fixed_rate(NULL, "apb_pclk",
					       NULL, CLK_IS_ROOT, 1000000);

	clk[clk_1mhz] = clk_register_fixed_rate(NULL, "clk_1mhz",
						NULL, CLK_IS_ROOT, 1000000);
}

 ...
 */
#define pvt_ioread32be(p)   \
	({ \
		unsigned int __v = \
			be32_to_cpu((__force __be32)__raw_readl(p));	\
		dsb(); __v; \
	})
#define pvt_iowrite32be(v, p) \
	({ dsb(); __raw_writel((__force __u32)cpu_to_be32(v), p); })
...
void ncp_ddr_shutdown(void *nca, void *apb, unsigned long ctl_244)
{
	 ...
	 */
	dbg_write(0xaaaa0001, (unsigned int *)(nca + 0x1200));

	/* write register value into CDAR[0] */
	pvt_iowrite32be(ctl_244, (unsigned int *)(nca + 0x1000));
	/* CDR2 - Node.target */
	pvt_iowrite32be(0x00002200, (unsigned int *)(nca + 0xf8));
	/* CDR1 - word offset 0xf4 (byte offset 0x3d0) */
	pvt_iowrite32be(0x000000f4, (unsigned int *)(nca + 0xf4));
	/* CDR0 - write command */
	pvt_iowrite32be(0x80050003, (unsigned int *)(nca + 0xf0));
	wfc_loop = 0;
	do {
		if (wfc_loop++ > WFC_TIMEOUT) {
			dbg_write(value, (unsigned int *)(nca + 0x11fc));
			dbg_write(0xffff0001, (unsigned int *)(nca + 0x1200));
			goto do_reset;
		}
		dbg_write(wfc_loop, (unsigned int *)(nca + 0x11f8));
		value = pvt_ioread32be((unsigned int *)
				       (nca + 0xf0));
	} while ((0x80000000UL & value));
	dbg_write(0xaaaa0002, (unsigned int *)(nca + 0x1200));
	...
	if (two_elms) {
		 ...
		 */
		/* CDR2 - Node.target */
		pvt_iowrite32be(0x00000f00, (unsigned int *)(nca + 0xf8));
		/* CDR0 - write command */
		pvt_iowrite32be(0x80050003, (unsigned int *)(nca + 0xf0));
		wfc_loop = 0;
		do {
			if (wfc_loop++ > WFC_TIMEOUT) {
				dbg_write(value,
					  (unsigned int *)(nca + 0x11fc));
				dbg_write(0xffff0002,
					  (unsigned int *)(nca + 0x1200));
				goto do_reset;
			}
			value = pvt_ioread32be((unsigned int *)
					       (nca + 0xf0));
	...
	dbg_write(0xaaaa0003, (unsigned int *)(nca + 0x1200));

	/*
	 * Poll for SMEM0 refresh-mode command completion
	 */
	/* CDR1 - word offset 0x104 (byte offset 0x410) */
	pvt_iowrite32be(0x00000104, (unsigned int *)(nca + 0xf4));
	/* CDR2 - Node.target */
	pvt_iowrite32be(0x00002200, (unsigned int *)(nca + 0xf8));
	ddr_loop = 0;
	do {
		if (ddr_loop++ > DDR_TIMEOUT) {
			dbg_write(value, (unsigned int *)(nca + 0x11fc));
			dbg_write(0xffff0003, (unsigned int *)(nca + 0x1200));
			goto do_reset;
		}
		pvt_iowrite32be(wfc_loop, (unsigned int *)
				(nca + 0x11f0));

		/* issue config ring read */
		pvt_iowrite32be(0x80040003, (unsigned int *)
				(nca + 0xf0));
		wfc_loop = 0;
		do {
			if (wfc_loop++ > WFC_TIMEOUT) {
	...
	dbg_write(0xaaaa0004, (unsigned int *)(nca + 0x1200));
	...
	dbg_write(0xaaaa0005, (unsigned int *)(nca + 0x1200));
	...
	dbg_write(0xaaaa0006, (unsigned int *)(nca + 0x1200));
do_reset:
	/*
	 * Issue Chip reset
	 */

...
/*
 * Author: Charlie Paul <charlie.paul@windriver.com>
 */
...
static bool pm_first_cpu_of_cluster(u32 cpu)
{
	...
	case (2):
	case (3):
		/* CPU 0 will never be turned off */
		break;
	case (4):
	case (5):
	case (6):
...
static void pm_dickens_logical_shutdown(u32 cluster)
{
	...
	writel(bit, dickens +
	       (0x10000 * DKN_DVM_DOMAIN_OFFSET) + DKN_MN_DVM_DOMAIN_CTL_CLR);

	/* Check for complete */
	retries = PM_WAIT_TIME;

	do {
		status = readl(dickens + (0x10000 * DKN_DVM_DOMAIN_OFFSET)
...
void pm_cpu_shutdown(u32 cpu)
{
	...
	if (last_cpu) {
		...
		pm_clear_bits_syscon_register(NSP_CSYSREQ_CNT, cluster_mask);
		success = pm_wait_for_bit_clear_with_timeout(NSP_CACTIVE_CNT,
							     cluster);
		if (!success) {
			pr_err("Failed to keep other cluster count going on cluster %d: %s-%d\n",
			       cluster, __FILE__, __LINE__);
			goto pm_shutdown_exit;
		}

		/* Turn off the ACE */
		pm_or_bits_syscon_register(NSP_ACEPWRDNRQ, cluster_mask);

		/* Wait for ACE to complete power off */
		success = pm_wait_for_bit_clear_with_timeout(NSP_NACEPWRDNACK,
							     cluster);
		if (!success) {
		...
		/* Isolate the cluster */
		pm_or_bits_syscon_register(NSP_ISOLATEL2MISC, cluster_mask);

		/* Wait for WFI L2 to go to standby */
		success = pm_test_for_bit_with_timeout(NSP_STANDBYWFIL2,
						       cluster);
		if (!success) {
...
inline void pm_cpu_logical_powerup(void)
{
	...
	asm volatile("	mrc		p15, 1, %0, c15, c0, 4\n"
		     "	bic	%0, %0, %1\n"
...
inline void pm_cluster_logical_powerup(void)
{
	 ...
	 */
	asm volatile("	mrc	p15, 1, r0, c15, c0, 0\n"
		     "	orr	%0, %0, %1\n"
		     "	orr	%0, %0, %2\n"
		     "	orr	%0, %0, %3\n"
		     "	orr	%0, %0, %4\n"
...
static int pm_cpu_physical_isolation_and_power_down(int cpu)
{
	...
	pm_clear_bits_syscon_register(NSP_PWRUPCPURAM, mask);

	/* Wait until the RAM power down is complete */
	success = pm_test_for_bit_with_timeout(NSP_NPWRUPCPURAM_ACK, cpu);
	if (!success) {
	...
	pm_or_bits_syscon_register(NSP_ISOLATECPU, mask);
	...
	success = pm_test_for_bit_with_timeout(NSP_NPWRUPCPUSTG1_ACK, cpu);
	if (!success) {
...
static int pm_cpu_physical_connection_and_power_up(int cpu)
{
	...
	pm_or_bits_syscon_register(NSP_PWRUPCPUSTG1, mask);

	/* Wait until CPU logic power is compete */
	success = pm_wait_for_bit_clear_with_timeout(NSP_NPWRUPCPUSTG1_ACK,
						     cpu);
	if (!success) {
	...
	success = pm_wait_for_bit_clear_with_timeout(NSP_NPWRUPCPURAM_ACK,
						     cpu);
	if (!success) {
		rval = -PM_ERR_RAM_ACK_FAIL;
		pr_err("CPU: Failed to get ACK of power up\n");
		goto power_up_cleanup;
	}
...
static void pm_L2_isolation_and_power_down(int cluster)
{
	...
	pm_or_bits_syscon_register(NSP_CHIPSELECTEN, mask);

	/* Disable the hsram */
	pm_clear_bits_syscon_register(NSP_PWRUPL2HSRAM, mask);
	...
	case (0):
		...
		usleep_range(20, 21);
#else
		pm_clear_bits_syscon_register(NSP_PWRUPL20RAM_PWRUPL2RAM2,
					      RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_clear_bits_syscon_register(NSP_PWRUPL20RAM_PWRUPL2RAM1,
					      RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_clear_bits_syscon_register(NSP_PWRUPL20RAM_PWRUPL2RAM0,
					      RAM_ALL_MASK);
		usleep_range(20, 21);
	...
	case (1):
		...
		usleep_range(20, 21);
#else
		pm_clear_bits_syscon_register(NSP_PWRUPL21RAM_PWRUPL2RAM2,
					      RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_clear_bits_syscon_register(NSP_PWRUPL21RAM_PWRUPL2RAM1,
					      RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_clear_bits_syscon_register(NSP_PWRUPL21RAM_PWRUPL2RAM0,
					      RAM_ALL_MASK);
		usleep_range(20, 21);
#endif
		break;
	case (2):
		...
		usleep_range(20, 21);
#else
		pm_clear_bits_syscon_register(NSP_PWRUPL22RAM_PWRUPL2RAM2,
					      RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_clear_bits_syscon_register(NSP_PWRUPL22RAM_PWRUPL2RAM1,
					      RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_clear_bits_syscon_register(NSP_PWRUPL22RAM_PWRUPL2RAM0,
					      RAM_ALL_MASK);
		usleep_range(20, 21);
#endif
		break;
	case (3):
		...
		usleep_range(20, 21);
#else
		pm_clear_bits_syscon_register(NSP_PWRUPL23RAM_PWRUPL2RAM2,
					      RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_clear_bits_syscon_register(NSP_PWRUPL23RAM_PWRUPL2RAM1,
					      RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_clear_bits_syscon_register(NSP_PWRUPL23RAM_PWRUPL2RAM0,
					      RAM_ALL_MASK);
		usleep_range(20, 21);
#endif
		break;
	...
	/* Power down stage 2 */
	pm_clear_bits_syscon_register(NSP_PWRUPL2LGCSTG2, mask);

	/* Power down stage 1 */
	pm_clear_bits_syscon_register(NSP_PWRUPL2LGCSTG1, mask);
}
...
static int pm_L2_physical_connection_and_power_up(u32 cluster)
{
	...
	/* Power up stage 1 */
	pm_or_bits_syscon_register(NSP_PWRUPL2LGCSTG1, mask);

	/* Wait for the stage 1 power up to complete */
	success = pm_wait_for_bit_clear_with_timeout(NSP_NPWRUPL2LGCSTG1_ACK,
						     cluster);
	if (!success) {
	...
	/* Power on stage 2 */
	pm_or_bits_syscon_register(NSP_PWRUPL2LGCSTG2, mask);

	/* Set the chip select */
	pm_or_bits_syscon_register(NSP_CHIPSELECTEN, mask);

	/* Power up the snoop ram */
	pm_or_bits_syscon_register(NSP_PWRUPL2HSRAM, mask);

	/* Wait for the stage 1 power up to complete */
	success = pm_wait_for_bit_clear_with_timeout(NSP_NPWRUPL2HSRAM_ACK,
						     cluster);
	if (!success) {
	...
	case (0):
		...
		usleep_range(20, 21);
#else
		pm_set_bits_syscon_register(NSP_PWRUPL20RAM_PWRUPL2RAM2,
					    RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_set_bits_syscon_register(NSP_PWRUPL20RAM_PWRUPL2RAM1,
					    RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_set_bits_syscon_register(NSP_PWRUPL20RAM_PWRUPL2RAM0,
					    RAM_ALL_MASK);
		usleep_range(20, 21);
	...
	case (1):
		...
		usleep_range(20, 21);
#else
		pm_set_bits_syscon_register(NSP_PWRUPL21RAM_PWRUPL2RAM2,
					    RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_set_bits_syscon_register(NSP_PWRUPL21RAM_PWRUPL2RAM1,
					    RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_set_bits_syscon_register(NSP_PWRUPL21RAM_PWRUPL2RAM0,
					    RAM_ALL_MASK);
		usleep_range(20, 21);
#endif
		break;
	case (2):
		...
		usleep_range(20, 21);
#else
		pm_set_bits_syscon_register(NSP_PWRUPL22RAM_PWRUPL2RAM2,
					    RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_set_bits_syscon_register(NSP_PWRUPL22RAM_PWRUPL2RAM1,
					    RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_set_bits_syscon_register(NSP_PWRUPL22RAM_PWRUPL2RAM0,
					    RAM_ALL_MASK);
		usleep_range(20, 21);
#endif
		break;
	case (3):
		...
		usleep_range(20, 21);
#else
		pm_set_bits_syscon_register(NSP_PWRUPL23RAM_PWRUPL2RAM2,
					    RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_set_bits_syscon_register(NSP_PWRUPL23RAM_PWRUPL2RAM1,
					    RAM_ALL_MASK);
		usleep_range(20, 21);
		pm_set_bits_syscon_register(NSP_PWRUPL23RAM_PWRUPL2RAM0,
					    RAM_ALL_MASK);
		usleep_range(20, 21);
#endif
		break;
	...
	pm_clear_bits_syscon_register(NSP_CHIPSELECTEN, mask);

	/* Release the isolation clamps */
	pm_clear_bits_syscon_register(NSP_ISOLATEL2MISC, mask);

	/* Turn the ACE bridge power on*/
	pm_clear_bits_syscon_register(NSP_ACEPWRDNRQ, mask);
...
static int pm_L2_logical_powerup(u32 cluster, u32 cpu)
{
	...
	pm_or_bits_syscon_register(NSP_CSYSREQ_CNT, mask);
	...
	/* start L2 */
	pm_clear_bits_syscon_register(NSP_ACINACTM, mask);
...
void pm_debug_read_pwr_registers(void)
{
	...
	pr_err("NSP_CLKEN: 0x%x\n", reg);
	reg = readl(syscon + NSP_ACINACTM);
	pr_err("NSP_ACINACTM: 0x%x\n", reg);
	reg = readl(syscon + 0x140c);
	pr_err("NSP_CHIPSELECTEN: 0x%x\n", reg);
	reg = readl(syscon + 0x1410);
	pr_err("NSP_CSYSREQ_TS: 0x%x\n", reg);
	reg = readl(syscon + 0x1414);
	pr_err("NSP_CSYSREQ_CNT: 0x%x\n", reg);
	reg = readl(syscon + 0x1418);
	pr_err("NSP_CSYSREQ_ATB: 0x%x\n", reg);
	reg = readl(syscon + 0x141c);
	pr_err("NSP_CSYSREQ_APB: 0x%x\n", reg);
	reg = readl(syscon + 0x1420);
	pr_err("NSP_PWRUPL2LGCSTG1: 0x%x\n", reg);
	reg = readl(syscon + 0x1424);
	pr_err("NSP_PWRUPL2LGCSTG2: 0x%x\n", reg);
	reg = readl(syscon + 0x1428);
	pr_err("NSP_PWRUPL2HSRAM: 0x%x\n", reg);
	reg = readl(syscon + 0x142c);
	pr_err("NSP_ACEPWRDNRQ: 0x%x\n", reg);
	reg = readl(syscon + 0x1430);
	pr_err("NSP_ISOLATEL2MIS: 0x%x\n", reg);
	reg = readl(syscon + 0x1438);
	pr_err("NSP_NPWRUPL2LGCSTG1_ACK: 0x%x\n", reg);
	reg = readl(syscon + 0x143c);
	pr_err("NSP_NPWRUPL2HSRAM_ACK: 0x%x\n", reg);
	reg = readl(syscon + 0x1440);
	pr_err("NSP_STANDBYWFIL2: 0x%x\n", reg);
	reg = readl(syscon + 0x1444);
	pr_err("NSP_CSYSACK_TS: 0x%x\n", reg);
	reg = readl(syscon + 0x1448);
	pr_err("NSP_CACTIVE_TS: 0x%x\n", reg);
	reg = readl(syscon + 0x144c);
	pr_err("NSP_CSYSACK_CNT: 0x%x\n", reg);
	reg = readl(syscon + 0x1450);
	pr_err("NSP_CACTIVE_CNT: 0x%x\n", reg);
	reg = readl(syscon + 0x1454);
	pr_err("NSP_CSYSACK_ATB: 0x%x\n", reg);
	reg = readl(syscon + 0x1458);
	pr_err("NSP_CACTIVE_ATB: 0x%x\n", reg);
	reg = readl(syscon + 0x145c);
	pr_err("NSP_CSYSACK_APB: 0x%x\n", reg);
	reg = readl(syscon + 0x1460);
	pr_err("NSP_CACTIVE_APB: 0x%x\n", reg);
	reg = readl(syscon + 0x1464);
	pr_err("NSP_NACEPWRDNACK: 0x%x\n", reg);
	reg = readl(syscon + 0x1468);
	pr_err("NSP_CACTIVEM_EAGM: 0x%x\n", reg);
	reg = readl(syscon + 0x146c);
	pr_err("NSP_CACTIVEM_EAGS: 0x%x\n", reg);
	reg = readl(syscon + 0x1470);
	pr_err("NSP_CACTIVES_EAGM: 0x%x\n", reg);
	reg = readl(syscon + 0x1474);
	pr_err("NSP_CACTIVES_EAGS: 0x%x\n", reg);
	reg = readl(syscon + 0x1480);
	pr_err("NSP_PWRUPCPUSTG1: 0x%x\n", reg);
	reg = readl(syscon + 0x1484);
	pr_err("NSP_PWRUPCPUSTG2: 0x%x\n", reg);
	reg = readl(syscon + 0x1488);
	pr_err("NSP_PWRUPCPURAM: 0x%x\n", reg);
	reg = readl(syscon + 0x148c);
	pr_err("NSP_ISOLATECPU: 0x%x\n", reg);
	reg = readl(syscon + 0x1490);
	pr_err("NSP_NPWRUPCPUSTG1_ACK: 0x%x\n", reg);
	reg = readl(syscon + 0x1494);
	pr_err("NSP_NPWRUPCPURAM_ACK: 0x%x\n", reg);
	reg = readl(syscon + 0x1498);
	pr_err("NSP_QACTIVE: 0x%x\n", reg);
	reg = readl(syscon + 0x149C);
	pr_err("NSP_STANDBYWFI: 0x%x\n", reg);
	reg = readl(syscon + 0x14A0);
	pr_err("NSP_STANDBYWFE: 0x%x\n", reg);
	reg = readl(syscon + 0x14A4);
	pr_err("NSP_DBGNOPWRDWN: 0x%x\n", reg);
	reg = readl(syscon + 0x14A8);
	pr_err("NSP_DBGPWRUPREQ: 0x%x\n", reg);
...
void pm_dump_L2_registers(void)
{
	...
	pr_err("NSP_PWRUPL20RAM_PWRUPL2RAM2: 0x%x\n", reg);
	reg = readl(syscon + 0x1584);
	pr_err("NSP_PWRUPL20RAM_PWRUPL2RAM1: 0x%x\n", reg);
	reg = readl(syscon + 0x1588);
	pr_err("NSP_PWRUPL20RAM_PWRUPL2RAM0: 0x%x\n", reg);
	reg = readl(syscon + 0x158c);
	pr_err("NSP_PWRUPL21RAM_PWRUPL2RAM2: 0x%x\n", reg);
	reg = readl(syscon + 0x1590);
	pr_err("NSP_PWRUPL21RAM_PWRUPL2RAM1: 0x%x\n", reg);
	reg = readl(syscon + 0x1594);
	pr_err("NSP_PWRUPL21RAM_PWRUPL2RAM0: 0x%x\n", reg);
	reg = readl(syscon + 0x1598);
	pr_err("NSP_PWRUPL22RAM_PWRUPL2RAM2: 0x%x\n", reg);
	reg = readl(syscon + 0x159c);
	pr_err("NSP_PWRUPL22RAM_PWRUPL2RAM1: 0x%x\n", reg);
	reg = readl(syscon + 0x15a0);
	pr_err("NSP_PWRUPL22RAM_PWRUPL2RAM0: 0x%x\n", reg);
	reg = readl(syscon + 0x15a4);
	pr_err("NSP_PWRUPL23RAM_PWRUPL2RAM2: 0x%x\n", reg);
	reg = readl(syscon + 0x15a8);
	pr_err("NSP_PWRUPL23RAM_PWRUPL2RAM1: 0x%x\n", reg);
	reg = readl(syscon + 0x15ac);
	pr_err("NSP_PWRUPL23RAM_PWRUPL2RAM0: 0x%x\n", reg);
}
...
void pm_dump_dickens(void)
{
	...
	for (i = 0; i < DKN_HNF_TOTAL_NODES; ++i) {
		status = readl(dickens + (0x10000 * (DKN_HNF_NODE_ID + i))
			       + DKN_HNF_SNOOP_DOMAIN_CTL);
		usleep_range(1, 2);

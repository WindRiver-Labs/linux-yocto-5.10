...
static int hp_set_status_check(void *data)
{
	...
	if (hp_status != priv->hp_active_low) {
		...
		snd_soc_dapm_disable_pin(snd_soc_component_get_dapm(priv->codec_dai->component), "Ext Spk");
		snd_soc_dapm_disable_pin(snd_soc_component_get_dapm(priv->codec_dai->component), "Main MIC");
		 ...
		 */
		snd_soc_component_update_bits(priv->codec_dai->component, WM8960_ADDCTL1, 3<<2, 1<<2);

		snd_kctl_jack_report(priv->snd_card, priv->headset_kctl, 1);
	} else {
		snprintf(buf, 32, "STATE=%d", 0);
		snd_soc_dapm_enable_pin(snd_soc_component_get_dapm(priv->codec_dai->component), "Ext Spk");
		snd_soc_dapm_enable_pin(snd_soc_component_get_dapm(priv->codec_dai->component), "Main MIC");
		ret = 0;
		 ...
		 */
		snd_soc_component_update_bits(priv->codec_dai->component, WM8960_ADDCTL1, 3<<2, 2<<2);
...
static void wm8960_init(struct snd_soc_card *card)
{
	 ...
	 */
	snd_soc_component_update_bits(codec_dai->component, WM8960_ADDCTL1, 3<<2, 1<<2);
}

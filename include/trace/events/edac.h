...
TRACE_EVENT(edac_l2cache_syndrome,
	...
	TP_STRUCT__entry(__field(u64, syndrome)
	),

	TP_fast_assign(__entry->syndrome = syndrome;
	),
...
TRACE_EVENT(edac_l2cache_counter,
	...
	TP_STRUCT__entry(__field(int, counter)
	),

	TP_fast_assign(__entry->counter = counter;
	),

...
static irqreturn_t thrd_irq_handler(int irq, void *data)
{
	 ...
	 */
	h->thrd_irq_fn(h);
	alcw(priv, h->irq_enab_reg_addr, h->irq_state_mask);
	return IRQ_HANDLED;
}
...
static irqreturn_t hw_irq_handler(int irq, void *data)
{
	 ...
	 */
	alcr(priv, h->irq_state_reg_addr, &state);
	state &= h->irq_state_mask;

	if (state) {
		alcw(priv, h->irq_enab_reg_addr, 0x0);
		      ...
		      const char *name)
{
	...
	if (h->irq_enab_reg_addr) {
		alcr(priv, h->irq_enab_reg_addr, &mask);
		mask |= h->irq_state_mask;
		alcw(priv, h->irq_state_reg_addr, mask);
		alcw(priv, h->irq_enab_reg_addr, mask);
	}
				...
				u32 misc_state)
{
	...
	alcr(priv, RIO_ESCSR(priv->port_ndx), &escsr);
	alcr(priv, EPC_IECSR(priv->port_ndx), &iecsr);

	/* clear latched state indications */
	/* Adding I2E to preserve idle sequence select bit which is R/w */
	alcw(priv, RIO_ESCSR(priv->port_ndx),
	...
	if (iecsr & EPC_IECSR_RETE) {
		...
		alcw(priv, EPC_IECSR(priv->port_ndx),
		     (iecsr & EPC_IECSR_RETE));
	}
	if (misc_state & AMST_INT) {
		alcr(priv, RAB_AMAST_STAT, &amast);
		...
		/* clear latched state */
		alcw(priv, RAB_AMAST_STAT, amast);
	}
	if (misc_state & ASLV_INT) {
		alcr(priv, RAB_ASLV_STAT_CMD,  &aslv_state);
		alcr(priv, RAB_ASLV_STAT_ADDR, &aslv_addr);
		if (aslv_state & RAB_ASLV_STAT_CMD_USUP) {
			dev_err(priv->dev, "AMBA Slave Unsupported Command\n");
			alcw(priv, RAB_ASLV_STAT_CMD,
...
static void pw_irq_handler(struct rio_irq_handler *h, u32 state)
{
	...
	while (noofpw) {
read_buff:
		alcr(priv, RAB_IB_PW_DATA, &msg_word);
			       ...
			       struct rio_pw_irq *pw_data)
{
	...
	dev_dbg(priv->dev, "(%s): flush %d words from pwbuff\n",
		__func__, noofpw);
	for (x = 0; x < noofpw; x++) {
		alcr(priv, RAB_IB_PW_DATA, &dummy);
...
static int enable_pw(struct rio_mport *mport)
{
	...
	alcr(priv, RAB_IB_PW_CSR, &rval);
	rval |= RAB_IB_PW_EN;
	axxia_rio_flush_pw(mport, RAB_IB_PW_NUMWORDS(rval), pw_data);
	alcw(priv, RAB_IB_PW_CSR, rval);
...
static void misc_irq_handler(struct rio_irq_handler *h)
{
	...
	alcr(priv, RAB_INTR_STAT_MISC, &misc_state);
	/*
	...
	alcw(priv, RAB_INTR_STAT_MISC, misc_state);
}
...
static void linkdown_irq_handler(struct rio_irq_handler *h/*, u32 state*/)
{
	...
	RIO_LRC32(mport, RAB_SRDS_STAT1, &rstate);
	RIO_LRC32(mport, RAB_SRDS_CTRL2, &val);
	RIO_LRC32(mport, RAB_SRDS_CTRL1, &val1);
	pr_info("Link Down: RAB_SRDS STAT1 = %x CTRL1 = %x CTRL2 = %x\n",
		rstate, val1, val);
	while (1) {
		alcr(priv, RIO_ESCSR(priv->port_ndx), &val);
		if (val & 0x2) {
			pr_info("Link up, Exiting Linkdown monitor Handler\n");
			RIO_LRC32(mport, RAB_SRDS_CTRL1, &val);
			RIO_LWC32(mport, RAB_SRDS_CTRL1, (val | 0x2));
			RIO_LWC32(mport, RAB_SRDS_CTRL2, LINK_DOWN_TIMEOUT);
			break;
		}
	}
	RIO_LRC32(mport, RAB_SRDS_STAT1, &rstate);
...
static void rpio_irq_handler(struct rio_irq_handler *h)
{
	...
	alcr(priv, RAB_INTR_STAT_RPIO, &rstate);
#if defined(CONFIG_AXXIA_RIO_STAT)
	...
	if (rstate &  RPIO_TRANS_FAILED) {
		...
		alcr(priv, RAB_RPIO_STAT, &rpio_stat);
		...
		alcw(priv, RAB_RPIO_STAT, rpio_stat);
#if defined(CONFIG_AXXIA_RIO_STAT)
		priv->rpio_failed_count++;
#endif
	}
	alcw(priv, RAB_INTR_STAT_RPIO, rstate);
}
...
static void apio_irq_handler(struct rio_irq_handler *h)
{
	...
	alcr(priv, RAB_INTR_STAT_APIO, &astate);
#if defined(CONFIG_AXXIA_RIO_STAT)
	...
	if (astate & APIO_TRANS_FAILED) {
		...
		alcr(priv, RAB_APIO_STAT, &apio_stat);
		...
		alcw(priv, RAB_APIO_STAT, apio_stat);
#if defined(CONFIG_AXXIA_RIO_STAT)
		priv->apio_failed_count++;
#endif
	}
	alcw(priv, RAB_INTR_STAT_APIO, astate);
}
...
void rx_db_handler(struct rio_mport *mport)
{
	...
	alcr(priv, RAB_IB_DB_CSR, &csr);
	num_msg = IB_DB_CSR_NUM_MSG(csr);

	for (; num_msg; num_msg--) {
		alcr(priv, RAB_IB_DB_INFO, &info);
...
void db_irq_handler(struct rio_irq_handler *h, u32 state)
{
	...
	if (state & OB_DB_DONE_INT) {
		...
		for (db = 0; db < MAX_OB_DB; db++) {
			alcr(priv, RAB_OB_DB_CSR(db), &csr);
 ...
 */
static inline int choose_ob_dme_static(struct rio_priv	*priv, int mbox_dest,
				       int buf_sz, struct rio_msg_dme **ob_dme)
{
...
static void release_mbox(struct kref *kref)
{
	...
	for (letter = 0; letter < RIO_MSG_MAX_LETTER; letter++) {
		if (mb->me[letter])
			alcw(priv, RAB_IB_DME_CTRL(mb->me[letter]->dme_no), 0);
	}
				 ...
				 struct rio_desc **descriptors)
{
	...
	rdesc = kcalloc(entries, sizeof(*rdesc), GFP_ATOMIC);
	if (!rdesc)
		goto err;
	rdescriptors = kcalloc(entries, sizeof(*rdescriptors), GFP_ATOMIC);
	if (!rdescriptors)
		goto err;
...
{
	struct rio_priv *priv = mport->priv;
	struct rio_msg_dme *me = kzalloc(sizeof(*me), GFP_KERNEL);
	int rc = 0;
...
enum hrtimer_restart ob_dme_tmr_handler(struct hrtimer *hr)
{
	...
	dme_no = me->dme_no;
	alcr(priv, RAB_OB_DME_STAT(dme_no),
	     &dme_stat);
	...
	if (dme_stat & (OB_DME_STAT_DESC_FETCH_ERR |
		...
		if (dme_stat & (OB_DME_STAT_DATA_TRANS_ERR |
			...
			if (dme_stat & OB_DME_STAT_RESP_TO)
				dev_err(priv->dev, "OB DME%d: Response Timeout Error\n",
					dme_no);
		}
		ob_dme_msg_handler(h, dme_no);
	}
	alcw(priv, RAB_OB_DME_STAT(dme_no), dme_stat);
			       ...
			       struct rio_tx_dme *dme_s, int dme_no)
{
	...
	for (i = 0, desc = me->desc; i < entries; i++, desc++) {
		if (!priv->intern_msg_desc) {
			...
			*((u32 *)DESC_TABLE_W3_MEM(me, i)) = 0;
			...
			dw1 = 0;
			dw2 = (u32)(desc->msg_phys >> 8) & 0x3fffffff;
			RIO_LWC32(mport, DESC_TABLE_W0(me->dres.start + i),
				  dw0);
			RIO_LWC32(mport, DESC_TABLE_W1(me->dres.start + i),
				  dw1);
			RIO_LWC32(mport, DESC_TABLE_W2(me->dres.start + i),
				  dw2);
			RIO_LWC32(mport, DESC_TABLE_W3(me->dres.start + i),
				  0);
		}
	}
	...
	if (!priv->intern_msg_desc) {
		...
		*((u32 *)DESC_TABLE_W3_MEM(me, i)) = dw3;
		...
		RIO_LRC32(mport, DESC_TABLE_W2(me->dres.start + i), &dw2);
		dw2 |= ((desc_chn_start >> 8) & 0xc0000000);
		dw3  = 0;
		RIO_LWC32(mport, DESC_TABLE_W0(me->dres.start + i), dw0);
		RIO_LWC32(mport, DESC_TABLE_W2(me->dres.start + i), dw2);
		RIO_LWC32(mport, DESC_TABLE_W3(me->dres.start + i), dw3);
	}
 ...
 *
 * To increase efficiecny the Descriptors are allocated and initialized during
...
static int open_outb_mbox_static(struct rio_mport *mport,
				 void *dev_id, int mbox_id, int entries,
				 int prio)
{
	int  rc = 0;
	int  dme_no, buf_sz = 0;
	...
	if (!test_bit(RIO_DME_MAPPED, &me->state)) {
		...
		alcw(priv, RAB_OB_DME_DESC_ADDR(dme_no), desc_addr);
		alcw(priv, RAB_OB_DME_CTRL(dme_no), dme_ctrl);
		me->dme_ctrl = dme_ctrl;
		me->dme_ctrl |= (DME_WAKEUP | DME_ENABLE);
		priv->ob_dme_irq.irq_state_mask |= (1 << dme_no);
		alcw(priv, RAB_INTR_STAT_ODME, 1 << dme_no);
		alcw(priv, RAB_INTR_ENAB_ODME, priv->ob_dme_irq.irq_state_mask);
...
static void release_outb_dme(struct rio_irq_handler *h)
{
	...
	for (i = 0; i < DME_MAX_OB_ENGINES; i++) {
		...
		if (me && test_bit(RIO_DME_OPEN, &me->state)) {
			if (test_bit(RIO_DME_MAPPED, &me->state)) {
				alcw(priv, RAB_OB_DME_CTRL(me->dme_no), 0);
...
static void  ib_dme_irq_handler(struct rio_irq_handler *h/*, u32 state*/)
{
	...
	u32 dme_mask, mask;
ib_dme_restart:
	alcr(priv, RAB_INTR_STAT_IDME, &dme_mask);
	mask = dme_mask;
	if (!mask)
		return;
	alcw(priv, RAB_INTR_STAT_IDME, mask);
	/**
	...
	while (dme_mask) {
		...
		alcw(priv, RAB_IB_DME_STAT(dme_no), dme_stat);
		if (!me)
			continue;
 ...
 *
 * Initializes buffer ring. Sets up descriptor ring and memory
			 ...
			 int mbox, int entries)
{
	...
	for (i = 0; i < RIO_MSG_MAX_LETTER; i++) {
		mb->virt_buffer[i] =
			kcalloc(mb->ring_size, sizeof(void *), GFP_KERNEL);
		if (!mb->virt_buffer[i]) {
			kfree(mb);
			return -ENOMEM;
		}
	...
	for (letter = 0; letter < RIO_MSG_MAX_LETTER; ++letter) {
		...
		if (!priv->intern_msg_desc) {
			...
			*((u32 *)DESC_TABLE_W3_MEM(me, i)) = dw3;
			...
			RIO_LRC32(mport, DESC_TABLE_W2(me->dres.start + i),
				  &dw2);
			dw3  = 0;
			dw2 |= ((desc_chn_start >> 8) & 0xc0000000);
			RIO_LWC32(mport, DESC_TABLE_W0(me->dres.start + i),
		...
		alcw(priv, RAB_IB_DME_DESC_ADDR(dme_no), desc_addr);
		alcw(priv, RAB_IB_DME_CTRL(dme_no), dme_ctrl);
	...
	AXXIA_RIO_SYSMEM_BARRIER();
	alcw(priv, RAB_INTR_STAT_IDME, irq_state_mask);
	...
	if (priv->dme_mode == AXXIA_IBDME_TIMER_MODE) {
		...
		alcw(priv, RAB_INTR_ENAB_IDME, h->irq_state_mask);
	}
...
void axxia_rio_port_get_state(struct rio_mport *mport, int cleanup)
{
	...
	if (cleanup) {
		...
		alcr(priv, RAB_INTR_STAT_MISC, &state);
		alcw(priv, RAB_INTR_STAT_MISC, state);
		/* Outbound Message Engine */
		alcr(priv, RAB_INTR_STAT_ODME, &state);
		alcw(priv, RAB_INTR_STAT_ODME, state);
		/* Inbound Message Engine */
		alcr(priv, RAB_INTR_STAT_IDME, &state);
		alcw(priv, RAB_INTR_STAT_IDME, state);
		/* Axxi Bus to RIO Events */
		alcr(priv, RAB_INTR_STAT_APIO, &state);
		alcw(priv, RAB_INTR_STAT_APIO, state);
		/* RIO to Axxia Bus Events */
		alcr(priv, RAB_INTR_STAT_RPIO, &state);
		alcw(priv, RAB_INTR_STAT_RPIO, state);
	}
...
int axxia_rio_port_irq_enable(struct rio_mport *mport)
{
	...
	RIO_LWC32(mport, RAB_SRDS_CTRL1, 0x0);
	RIO_LWC32(mport, RAB_SRDS_CTRL2, /*LINK_DOWN_TIMEOUT*/0x0);
	...
	if (priv->dme_mode == AXXIA_IBDME_INTERRUPT_MODE) {
		...
		alcw(priv, RAB_INTR_ENAB_GNRL,
		     (RAB_INTR_ENAB_GNRL_SET | IB_DME_INT_EN));
	} else {
		alcw(priv, RAB_INTR_ENAB_GNRL, RAB_INTR_ENAB_GNRL_SET);
	}
out:
	return rc;
err0:
...
void axxia_rio_port_irq_disable(struct rio_mport *mport)
{
	 ...
	 * Mask top level IRQs
	 */
	alcw(priv, RAB_INTR_ENAB_GNRL, 0);
	/**
	...
	for (db = 0; db < MAX_OB_DB; db++) {
		...
		if (OB_DB_STATUS(csr) == OB_DB_STATUS_DONE &&
			...
			dev_dbg(priv->dev,
				...
				data, destid);
			alcw(priv, RAB_OB_DB_INFO(db), OB_DB_INFO(data));
			alcw(priv, RAB_OB_DB_CSR(db), csr);
			break;
		}
	}
	if (db == MAX_OB_DB)
...
int axxia_open_outb_mbox(struct rio_mport *mport, void *dev_id, int mbox_dme,
			 int entries)
{
	struct rio_priv *priv = mport->priv;
	int rc = 0;
			   ...
			   void *buffer, size_t len/*, void *cookie*/)
{
	...
	*((u32 *)DESC_TABLE_W0_MEM(me, idx)) = dw0;
	...
	alcw(priv, RAB_OB_DME_CTRL(me->dme_no), me->dme_ctrl);
...
void axxia_close_inb_mbox(struct rio_mport *mport, int mbox)
{
	...
	alcw(priv, RAB_INTR_ENAB_IDME, priv->ib_dme_irq.irq_state_mask);
	alcw(priv, RAB_INTR_STAT_IDME, mb->irq_state_mask);
	msleep(500);
	...
	for (letter = 0; letter < RIO_MSG_MAX_LETTER; letter++) {
		...
		if (mb->me[letter]) {
			...
			alcw(priv, RAB_IB_DME_CTRL(dme_no), 0);
		}
	}
	alcw(priv, RAB_INTR_STAT_IDME, mb->irq_state_mask);
	mb->irq_state_mask = 0;
...
int axxia_add_inb_buffer(struct rio_mport *mport, int mbox, void *buf)
{
	...
	*((u32 *)DESC_TABLE_W0_MEM(me, me->write_idx)) = dw0;
	AXXIA_RIO_SYSMEM_BARRIER();
	me->write_idx = (me->write_idx + 1) & (me->entries - 1);
	alcw(priv, RAB_IB_DME_CTRL(me->dme_no), me->dme_ctrl);
done:
	return rc;
busy:
	rc = -EBUSY;

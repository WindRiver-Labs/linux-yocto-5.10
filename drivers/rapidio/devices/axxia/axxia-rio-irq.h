...
#define	RIO_MBOX_TO_BUF_SIZE(mid) \
	(((mid) <= RIO_MAX_RX_MBOX_4KB) ? \
	 RIO_MSG_MULTI_SIZE : RIO_MSG_SEG_SIZE)
#define	RIO_OUTB_DME_TO_BUF_SIZE(p, did) \
	(((did) < (p)->num_outb_dmes[0]) ? \
	 RIO_MSG_MULTI_SIZE : RIO_MSG_SEG_SIZE)
...
struct rio_msg_dme {
	spinlock_t lock;	/* SRIO Message DME Lock */
	unsigned long state;
	struct kref kref;
...
struct rio_rx_mbox {
	spinlock_t lock;	/* SRIO RX MBOX Lock */
	unsigned long state;
	int mbox_no;
	struct kref kref;
...
struct rio_tx_mbox {
	spinlock_t lock;	/* SRIO TX MBOX Lock */
	unsigned long state;
	struct rio_mport *mport;
	int mbox_no;
	int dme_no;
...
int alloc_irq_handler(struct rio_irq_handler *h, void *data, const char *name);

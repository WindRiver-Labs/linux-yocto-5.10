...
/**
 * __alcr - Generate a AXXIA local config space read
...
/**
 * alcw - Generate a AXXIA local config space write
 * @priv: Master port private data
 * @offset: Offset into configuration space
			  ...
			  int len, u32 *val)
{
	...
	addr = (u8 *)aoutb->win + (offset & (CONFIG_RIO_MAINT_WIN_SIZE - 1));
			   ...
			   int len, u32 val)
{
	...
	alcw(priv,
	     ...
	     ctrl);
	rval = val;
	data = (u8 *)aoutb->win + (offset & (CONFIG_RIO_MAINT_WIN_SIZE - 1));
	...
	if (error_code != 0) {
		dev_dbg(priv->dev, "%s: Error when writing\n", __func__);
				     ...
				     u32 mflags, u32 *id)
{
	...
	riores = kzalloc(sizeof(*riores), GFP_ATOMIC);
	if (!riores) {
		aoutb->in_use = 0;
		spin_unlock_irqrestore(&rio_io_lock, flags);
		dev_err(priv->dev,
	...
	RIO_LWC32(mport, RAB_APIO_AMAP_ABAR(win), wabar);

	if (WIN_SIZE((u32)size) == 0)
		size = 0x400u; /* make sure window size is at least 1KiB big*/

	/* Set size of window */
	win_size |= WIN_SIZE((u32)size);
	RIO_LWC32(mport, RAB_APIO_AMAP_SIZE(win), win_size);
	RIO_LRC32(mport, RAB_APIO_AMAP_CTRL(win), &reg);
	reg &= ~TTYPE(0x3);
	reg |= trans_type;
	RIO_LWC32(mport, RAB_APIO_AMAP_CTRL(win), reg);
					  ...
					  u32 win)
{
	...
	RIO_LRC32(mport, RAB_APIO_AMAP_CTRL(win), &ctrl);
	if (likely(priv->outb_atmu[win].in_use)) {
		...
		RIO_LWC32(mport, RAB_APIO_AMAP_CTRL(win), ctrl & ~ENABLE_AMBA);
		aoutb->riores = NULL;
		aoutb->win = NULL;
...
void axxia_rio_set_mport_disc_mode(struct rio_mport *mport)
{
	...
	if (mport->host_deviceid >= 0) {
		RIO_LWC32(mport, RIO_GCCSR,
			  RIO_PORT_GEN_HOST |
			  RIO_PORT_GEN_MASTER |
			  RIO_PORT_GEN_DISCOVERED);
	} else {
		RIO_LWC32(mport, RIO_GCCSR, RIO_PORT_GEN_MASTER);
		RIO_LWC32(mport, RIO_DID_CSR,
			  RIO_SET_DID(mport->sys_size,
				      RIO_ANY_DESTID(mport->sys_size)));
	}

#ifdef EXTRA1DEBUG
	RIO_LRC32(mport, RIO_GCCSR, &result);
	EXT1P("rio[%d]: RIO_GEN_CTL_CSR set to 0x%X for main port\n",
	      mport->id, result);
#endif

	RIO_LWC32(mport, RIO_COMPONENT_TAG_CSR, 0xFFFF);
	...
	if (mport->phy_type == RIO_PHY_SERIAL)
		RIO_LWC32(mport, EPC_PNPTAACR(mport->port_ndx), 0x00000000);
#endif
	...
	if (CONFIG_RAPIDIO_SECOND_DEST_ID != DESTID_INVALID) {
		...
		result |=
			EPC_PNADIDCSR_ADID_SMALL(CONFIG_RAPIDIO_SECOND_DEST_ID);
		RIO_LWC32(mport, EPC_PNADIDCSR(priv->port_ndx), result);
...
static void axxia_init_port_data(struct rio_mport *mport)
{
	...
	RIO_LRC32(mport, RAB_SRDS_CTRL0, &data);
	RIO_LWC32(mport, RAB_SRDS_CTRL0, data | RAB_SRDS_CTRL0_16B_ID);
#endif
	/* Probe the master port phy type */
	RIO_LRC32(mport, RIO_CCSR(priv->port_ndx), &ccsr);
	...
	RIO_LRC32(mport, RIO_PEF_CAR, &data);
	mport->sys_size = (data & RIO_PEF_CTLS) >> 4;
	dev_dbg(priv->dev, "RapidIO Common Transport System size: %d\n",
		mport->sys_size ? 65536 : 256);

	RIO_LRC32(mport, RIO_DEV_ID_CAR, &priv->devid);
	RIO_LRC32(mport, RIO_DEV_INFO_CAR, &priv->devrev);
	{
		int i;
...
static int rio_start_port(struct rio_mport *mport)
{
	...
	/* Probe the master port phy type */
	RIO_LRC32(mport, RIO_CCSR(priv->port_ndx), &ccsr);
	RIO_LRC32(mport, RIO_ESCSR(priv->port_ndx), &escsr);
	...
	if (escsr & RIO_ESCSR_PU) {
		...
		/* Disable ports */
		ccsr |= RIO_CCSR_PD;
		RIO_LWC32(mport, RIO_CCSR(priv->port_ndx), ccsr);
...
#ifdef EXTRA1DEBUG
	{
		...
		RIO_LRC32(mport, RIO_HOST_DID_LOCK_CSR, &hdlcsr);
		RIO_LRC32(mport, RAB_VER, &rabver);
...
static void rio_rab_ctrl_setup(struct rio_mport *mport)
{
	...
	RIO_LWC32(mport, AXI_TIMEOUT, 0x00001000);

#ifdef USE_DME_TIMEOUT
	RIO_LWC32(mport, DME_TIMEOUT, 0xC0080000);
#else
	RIO_LWC32(mport, DME_TIMEOUT, 0x00000000);
#endif
	...
	rab_ctrl |= (2 << 6);
	rab_ctrl |= 3;
	RIO_LWC32(mport, RAB_CTRL, rab_ctrl);
}
...
{
	RIO_LWC32(mport, RAB_APIO_CTRL,
		  RAB_APIO_MAINT_MAP_EN |
		  RAB_APIO_MEM_MAP_EN |
		  RAB_APIO_PIO_EN);
	RIO_LWC32(mport, RAB_RPIO_CTRL, RAB_RPIO_PIO_EN);
}
...
int axxia_rio_static_win_init(struct rio_mport *mport)
{
	...
	RIO_LWC32(mport, RAB_IB_DB_CSR, IB_DB_CSR_EN);
	...
	RIO_LRC32(mport, RAB_APIO_AMAP_CTRL(priv->maint_win_id), &ctrl);
	/* Enable window */
	ctrl |= ENABLE_AMBA;
	RIO_LWC32(mport, RAB_APIO_AMAP_CTRL(priv->maint_win_id), ctrl);
...
void axxia_rio_static_win_release(struct rio_mport *mport)
{
	...
	RIO_LRC32(mport, RAB_IB_DB_CSR, &ibdb);
	ibdb &= ~IB_DB_CSR_EN;
	RIO_LWC32(mport, RAB_IB_DB_CSR, ibdb);
...
static int rio_parse_dtb(struct platform_device *dev, int *ndx, u64 *law_start,
			 ...
			 int *outb_dmes, int *ib_num_dmes, int *inb_dmes,
			 int *irq, struct event_regs *linkdown_reset)
{
...
rio_priv_dtb_setup(struct platform_device *dev,	struct resource *regs,
		   struct rio_mport *mport, int ndx, int port_ndx,
		   int *num_outb_dmes, int *outb_dmes, int *num_inb_dmes,
		   int *inb_dmes, int irq, struct event_regs *linkdown_reset)
{
...
int axxia_rio_start_port(struct rio_mport *mport)
{
	...
	/* Miscellaneous
	 */
	RIO_LWC32(mport, RAB_OB_DME_TID_MASK, OB_DME_TID_MASK);
 ...
 *    initialized. At this stage No HW access is possible, to avoid
...
static int axxia_rio_setup(struct platform_device *dev)
{
	...
	int num_ob_dmes[2] = { 0, }, outb_dmes[2] = { 0, };
	int num_ib_dmes[2] = { 0, }, inb_dmes[2] = { 0, };
	...
	if (rio_parse_dtb(dev, &ndx, &law_start, &law_size, &regs,
			  &num_ob_dmes[0], &outb_dmes[0],
			  &num_ib_dmes[0], &inb_dmes[0],
			  &irq, &linkdown_reset))
		return -EFAULT;
	...
	case AXXIA_DEVID_AXM55XX:
		...
		case AXXIA_DEVREV_AXM55XX_V1_0:
			pr_info("RIO: AXM 55xx sRIO Dev Rev 0 (Base DID Lock issue)\n");
			break;
		case AXXIA_DEVREV_AXM55XX_V1_1:
			pr_info("RIO: AXM 55xx sRIO Device Rev 1 Controller %d\n",
				priv->ndx);
			break;
		case AXXIA_DEVREV_AXM55XX_V1_2:
			pr_info("RIO: AXM 55xx sRIO Device Rev 2 Controller %d\n",
				priv->ndx);
			break;
		default:
			break;

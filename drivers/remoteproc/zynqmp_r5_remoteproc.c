...
static int r5_set_mode(struct zynqmp_r5_pdata *pdata)
{
	...
	ret = zynqmp_pm_get_rpu_oper_mode(val);
	if (ret < 0) {
	...
	if (val[0] == expect) {
		...
		ret = zynqmp_pm_set_rpu_oper_mode(expect, val);
		if (ret < 0) {
			dev_err(dev,
	...
	ret = zynqmp_pm_tcm_comb_config(expect, val);
	if (ret < 0) {
...
static int zynqmp_r5_rproc_start(struct rproc *rproc)
{
	...
	ret = zynqmp_pm_request_wakeup(local->pnode_id, 1, bootmem,

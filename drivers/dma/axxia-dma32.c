...
static void soft_reset(struct gpdma_engine *engine)
{
	...
	/* Reset all channels */
	for (i = 0;
	     i < engine->chip->num_channels && i < MAX_GPDMA_CHANNELS; i++)
			    ...
			    size_t len)
{
	...
	WARN_ON(src_count * (1 << src_acc) != len);

	if (src_count * (1 << src_acc) != len)
		return;

	WARN_ON(dst_count * (1 << dst_acc) != len);

	if (dst_count * (1 << dst_acc) != len)
		return;
...
soft_reset_store(struct device *dev,
...
/**
 * gpdma_prep_sg - Prepares a slave transfer using sg lists.
 *
 */
static struct dma_async_tx_descriptor *
gpdma_prep_slave_sg(struct dma_chan *chan, struct scatterlist *sgl,
		    ...
		    unsigned long dma_flags, void *context)
{
	...
	return gpdma_prep_sg(chan, sgl, sg_len, sgl, sg_len, dma_flags);
}
...
static int gpdma_of_probe(struct platform_device *op)
{
	...
	dma->device_prep_slave_sg = gpdma_prep_slave_sg;

...
module_param(enable_los_wa, int, 0660);
...
axxia_pcie_readl_rc(struct pcie_port *pp, u32 reg, u32 *val)
{
	*val = readl(pp->dbi_base + reg);
	...
	if (trace != 0)
		pr_info("PEI%d: Read 0x%x from DBI + 0x%x\n",
			pp->pei_nr, *val, reg);
}
...
axxia_pcie_writel_rc(struct pcie_port *pp, u32 val, u32 reg)
{
	...
	if (trace != 0)
		pr_info("PEI%d: Wrote 0x%x to DBI + 0x%x\n",
			pp->pei_nr, val, reg);
}
...
axxia_cc_gpreg_writel(struct pcie_port *pp, u32 val, u32 reg)
{
	...
	if (trace != 0)
		pr_info("PEI%d: Wrote 0x%x to CC_GPREG + 0x%x\n",
			pp->pei_nr, val, reg);
}
...
axxia_cc_gpreg_readl(struct pcie_port *pp, u32 reg, u32 *val)
{
	*val = readl(pp->cc_gpreg_base + reg);
	...
	if (trace != 0)
		pr_info("PEI%d: Read 0x%x from CC_GPREG + 0x%x\n",
			pp->pei_nr, *val, reg);
}
...
axxia_axi_gpreg_writel(struct pcie_port *pp, u32 val, u32 reg)
{
	...
	if (trace != 0)
		pr_info("PEI%d: Wrote 0x%x to AXI_GPREG + 0x%x\n",
			pp->pei_nr, val, reg);
}
...
axxia_axi_gpreg_readl(struct pcie_port *pp, u32 reg, u32 *val)
{
	*val = readl(pp->axi_gpreg_base + reg);
	...
	if (trace != 0)
		pr_info("PEI%d: Read 0x%x from AXI_GPREG + 0x%x\n",
			pp->pei_nr, *val, reg);
}
...
axxia_pcie_rd_own_conf(struct pcie_port *pp, int where, int size, u32 *val)
{
	...
	if (trace != 0)
		pr_info("PEI%d: rd_own_conf where=%d size=%d ret=%d *val=0x%x\n",
				  ...
				  int size, u32 val)
{
	...
	ret = axxia_pcie_cfg_write(pp->dbi_base + (where & ~0x3), where, size,
				   val);

	if (trace != 0)
		pr_info("PEI%d: wr_own_conf where=%d size=%d ret=%d val=0x%x\n",
			pp->pei_nr, where, size, ret, val);
	...
	if (pp->msi_mode == AXXIA_MSI_UNCONFIGURED) {
		...
		if ((is_msix && pp->msi_mode == AXXIA_MSI_MODE) ||
			...
			dev_info(pp->dev,
				 ...
				 pp->msi_mode ==
				 AXXIA_MSI_MODE ? "MSI" : "MSIX",
				 pp->msi_mode ==
				 AXXIA_MSI_MODE ? "MSIX" : "MSI");
...
static void axxia_pcie_enable_interrupts(struct pcie_port *pp)
{
	...
	if (IS_ENABLED(CONFIG_PCI_MSI) && pp->num_msi_irqs == 0) {
#ifdef AXM_LEVEL_MSI
		axxia_cc_gpreg_readl(pp, CC_GPREG_LVL_IRQ_MASK, &val);
		val |= MSI_CNTRL_INT;
		axxia_cc_gpreg_writel(pp, val, CC_GPREG_LVL_IRQ_MASK);
#else
		axxia_cc_gpreg_readl(pp, CC_GPREG_EDG_IRQ_MASK_HI, &val);
		val |= MSI_ASSERTED;
		axxia_cc_gpreg_writel(pp, val, CC_GPREG_EDG_IRQ_MASK_HI);
#endif
		axxia_axi_gpreg_readl(pp, AXI_GPREG_EDG_IRQ_MASK_HI, &val);
		val |= MSIX_ASSERTED;
		axxia_axi_gpreg_writel(pp, val, AXI_GPREG_EDG_IRQ_MASK_HI);
		axxia_pcie_msi_init(pp);
	} else if (IS_ENABLED(CONFIG_PCI_MSI) && pp->num_msi_irqs != 0) {
...
axxia_pcie_los_wa(struct pcie_port *pp, unsigned int max_width)
{
	...
	if (max_width != 4 && max_width != 2 && max_width != 1)
		return -1;

	/* If PEI0, make sure it is configured as the root complex. */
	if (pp->pei_nr == 0 && (control & 0x80) == 0)
		return 0;
...
static irqreturn_t axxia_pcie_irq_handler(int irq, void *arg)
{
	...
	if (IS_ENABLED(CONFIG_PCI_MSI) && pp->num_msi_irqs == 0) {
		offset = irq - pp->msi_irqs[0];
#ifdef AXM_LEVEL_MSI
		axxia_cc_gpreg_readl(pp, CC_GPREG_LVL_IRQ_STAT, &val);

		if (val & MSI_CNTRL_INT)
#else
			axxia_cc_gpreg_readl(pp, CC_GPREG_EDG_IRQ_STAT_HI,
					     &val);
...
#endif
		{
			axxia_pcie_rd_own_conf(pp, PCIE_MSI_INTR0_STATUS, 4,
					       (u32 *)&val1);
			if (val1)
				ret = axxia_dw_pcie_handle_msi_irq(pp, val1);
		...
		axxia_axi_gpreg_readl(pp, AXI_GPREG_EDG_IRQ_STAT_HI, &val);
		...
		if (val & MSIX_ASSERTED) {
			...
			for (i = 0 ; i < 32; i++) {
				if ((val1) & (1 << i))
					ret = axxia_dw_pcie_handle_msix_irq(pp,
									    i);
			}
...
static int assign_irq(int no_irqs, struct msi_desc *desc, int *pos)
{
	...
	u32 is_msix = 0;
	struct pcie_port *pp =
		(struct pcie_port *)msi_desc_to_pci_sysdata(desc);
...
int axxia_pcie_host_init(struct pcie_port *pp)
{
	...
	if (IS_ENABLED(CONFIG_PCI_MSI)) {
		pp->irq_domain =
			irq_domain_add_linear(pp->dev->of_node,
					      MAX_MSI_IRQS,
...
static int axxia_pcie_probe(struct platform_device *pdev)
{
	...
	if (!res) {
		devm_kfree(&pdev->dev, axxia_pcie);
		return -ENOMEM;
	}

	if (res->start == 0xa002000000) {
		pp->pei_nr = 0;
	} else if (res->start == 0xa004000000) {
		pp->pei_nr = 1;
	} else if (res->start == 0xa006000000) {
		pp->pei_nr = 2;
	} else {
		pr_warn("Unknown PEI!\n");
		pp->pei_nr = -1;

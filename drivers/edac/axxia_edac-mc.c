...
/*
 * EDAC Driver for Intels Axxia 5600 Memory Controller
 */
...
module_param(machine_restrt, int, 0644);
 ...
 *	      Error information can be found
		...
		if ((status & event_mask[i]) != 0) {
			if (machine_restrt && event_logging[i].fatal) {
				if (IS_ERR(edac_dev->syscon)) {
					set_val = readl(edac_dev->apb2ser3_region +

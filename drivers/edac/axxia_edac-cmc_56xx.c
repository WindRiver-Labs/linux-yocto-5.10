		...
		if ((denali_ctl_84->int_status & event_mask[i]) != 0) {
			if (force_restart && event_logging[i].fatal) {
				if (IS_ERR(edac_dev->syscon)) {
					set_val = readl(edac_dev->axi2ser3_region
		...
		for (j = 0; j < cs_count; ++j) {
			for (l = 0; l < dram_count; ++l) {
				for (k = 0; k < MPR_ERRORS; ++k, ++i) {
					struct event_counter *ptr;
					int idx =  NR_EVENTS +
						MAX_DQ * MPR_ERRORS * j +
						MPR_ERRORS * l + k;

					ptr = &dev_info->data->alerts[j][l][k];
					ptr->edac_block_idx = i;
					block = &instance->blocks[i];
					snprintf(block->name,
						 sizeof(block->name),
						 "%s", block_name[idx]);
...
static const struct proc_ops axxia_edac_cmem_proc_ops = {
	.proc_open       = axxia_cmem_open,
	.proc_read       = axxia_cmem_read,
	.proc_write      = axxia_cmem_write,
	.proc_release    = axxia_cmem_close,
...
static int intel_edac_mc_probe(struct platform_device *pdev)
{
	...
	for (j = 0; j < MAX_CS; ++j) {
		for (l = 0; l < MAX_DQ; ++l) {
			for (k = 0; k < MPR_ERRORS; ++k, ++i)
				atomic_set(&dev_info->data->alerts[j][l][k].counter, 0);
		}
	}

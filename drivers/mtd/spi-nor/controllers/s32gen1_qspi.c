...
int s32gen1_qspi_read_reg(struct spi_nor *nor, u8 opcode, u8 *buf, size_t len)
{
...
int s32gen1_qspi_write_reg(struct spi_nor *nor, u8 opcode, const u8 *buf, size_t len)
{
	struct spi_mem_op op = SPI_MEM_OP(SPI_MEM_OP_CMD(opcode, 1),
			SPI_MEM_OP_NO_ADDR,
			SPI_MEM_OP_NO_DUMMY,
	...
	return s32gen1_qspi_read_write_reg(nor, &op, (u8 *)buf);
}

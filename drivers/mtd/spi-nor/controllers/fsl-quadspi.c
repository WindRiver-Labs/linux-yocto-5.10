...
#ifndef CONFIG_SOC_S32GEN1
static int fsl_qspi_read_reg(struct spi_nor *nor, u8 opcode, u8 *buf, size_t len)
{
	int ret;
...
static int fsl_qspi_write_reg(struct spi_nor *nor, u8 opcode, const u8 *buf, size_t len)
{
	struct fsl_qspi *q = nor->priv;
	int ret;
...
static int fsl_qspi_prep(struct spi_nor *nor)
{
	struct fsl_qspi *q = nor->priv;
	int ret;
...
static void fsl_qspi_unprep(struct spi_nor *nor)
{
	struct fsl_qspi *q = nor->priv;
...
#ifdef CONFIG_SOC_S32GEN1
static const struct spi_nor_controller_ops s32gen1_qspi_controller_ops = {
	.read_reg  = s32gen1_qspi_read_reg,
	.write_reg = s32gen1_qspi_write_reg,
	.read  = s32gen1_qspi_read,
	.write = s32gen1_qspi_write,
	.erase = fsl_qspi_erase,
	.prepare = fsl_qspi_prep,
	.unprepare = fsl_qspi_unprep,
};
#else
static const struct spi_nor_controller_ops fsl_qspi_controller_ops = {
	.read_reg  = fsl_qspi_read_reg,
	.write_reg = fsl_qspi_write_reg,
	.read  = fsl_qspi_read,
...
static int fsl_qspi_probe(struct platform_device *pdev)
{
	...
	for_each_available_child_of_node(dev->of_node, np) {
		...
		/* fill the hooks */
#ifdef CONFIG_SOC_S32GEN1
		nor->controller_ops = &s32gen1_qspi_controller_ops;
#else
		nor->controller_ops = &fsl_qspi_controller_ops;
#endif

...
module_param(tx_num_desc, int, 0400);
...
#define D_EOP		BIT(4)
#define D_INTR		BIT(5)
#define D_RX_ERR	BIT(6)
#define D_TX_NEGCRC	BIT(6)
#define D_SWAP		BIT(7)
#define D_TX_CRC	BIT(8)
...
static ssize_t counters_show(struct device *d,
...
static ssize_t pti_show(struct device *d,
			struct device_attribute *attr,
			char *buf)
{
	struct nemac_priv *priv = netdev_priv(to_net_dev(d));
...
nemac_tx_cleanup(struct nemac_priv *priv)
{
	...
	while ((desc = queue_get_tail(&priv->txq)) != NULL) {
		...
		dev_kfree_skb_any(skb);
		mb();		/* Make Sure Hardware Has Been Updated! */

...
int ubootenv_get(const char *, char *);
#endif
...
struct appnic_device {
	...
	unsigned int tx_num_desc;

	/* buffers */
	unsigned int rx_buf_sz;
	unsigned int rx_buf_per_desc;
	void *rx_buf;
	dma_addr_t rx_buf_dma;
	unsigned int tx_buf_sz;
	unsigned int tx_buf_per_desc;
	void *tx_buf;
...
femac_irq_setup(struct net_device *dev)
{
	...
	if (irq_set_irq_type(dev->irq, IRQ_TYPE_LEVEL_HIGH) != 0)
		return -EINVAL;
#endif
	return 0;
}

...
do {									       \
	...
	NPC_SCAN_HDR(NPC_ETYPE_TAG2, NPC_LID_LB, NPC_LT_LB_STAG_QINQ, 6, 2);
	NPC_SCAN_HDR(NPC_VLAN_TAG1, NPC_LID_LB, NPC_LT_LB_CTAG, 0, 2);
	NPC_SCAN_HDR(NPC_VLAN_TAG2, NPC_LID_LB, NPC_LT_LB_STAG_QINQ, 0, 2);
}
				      ...
				      struct npc_install_flow_rsp *rsp)
{
	...
	err = nix_get_nixlf(rvu, target, &nixlf, &blkaddr);
	if (err)
		return err;

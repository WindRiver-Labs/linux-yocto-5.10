...
static inline void flexcan_request_fd(struct device *dev, bool *allowed)
{
...
static int flexcan_probe(struct platform_device *pdev)
{
	...
	u8 clk_src = 1;
	u32 clock_freq = 0;
	bool fd_allowed = true, named_irqs;
	...
	flexcan_request_fd(&pdev->dev, &fd_allowed);

	if ((priv->devtype_data->quirks & FLEXCAN_QUIRK_SUPPORT_FD) && fd_allowed) {

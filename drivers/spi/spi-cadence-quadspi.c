				   ...
				   struct spi_mem *mem)
{
	...
	read_data = kmalloc(ARRAY_SIZE(phy_tuning_pattern), GFP_KERNEL);
	if (!read_data)
		return -ENOMEM;
			       ...
			       struct spi_mem *mem)
{
	...
	if (bottomleft.read_delay == topright.read_delay) {
		...
		if (ret) {
			 ...
			 * the thermal sensor.
			 *
			 * TODO: Change it to dev_warn once we add support for
			  ...
			  const struct spi_mem_op *op)
{
	...
	/*
	 * If the command requires a frequency lower than our ref clock, we
	 * can't use PHY since PHY reads run at exactly the ref clock speed.
	 */
	if (op->max_freq != 0 && op->max_freq < cqspi->master_ref_clk_hz)
		return false;
...
static void cqspi_mem_phy_op(struct spi_mem *mem,
...
static const struct spi_controller_mem_ops cqspi_mem_ops = {
	...
	.set_calibration_read_op = cqspi_mem_phy_op,
};

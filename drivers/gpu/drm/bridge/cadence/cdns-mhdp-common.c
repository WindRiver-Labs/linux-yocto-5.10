		...
		struct audio_info *audio)
{
	...
	cdns_mhdp_bus_write(0, mhdp->regs + AUDIO_SRC_CNTL);
	cdns_mhdp_bus_write(0, mhdp->regs + AUDIO_SRC_CNFG);
	cdns_mhdp_bus_write(AUDIO_SW_RST, mhdp->regs + AUDIO_SRC_CNTL);
	cdns_mhdp_bus_write(0, mhdp->regs + AUDIO_SRC_CNTL);

	/* reset smpl2pckt component  */
	cdns_mhdp_bus_write(0, mhdp->regs + SMPL2PKT_CNTL);
	cdns_mhdp_bus_write(AUDIO_SW_RST, mhdp->regs + SMPL2PKT_CNTL);
	cdns_mhdp_bus_write(0, mhdp->regs + SMPL2PKT_CNTL);

	/* reset FIFO */
	cdns_mhdp_bus_write(AUDIO_SW_RST, mhdp->regs + FIFO_CNTL);
	cdns_mhdp_bus_write(0, mhdp->regs + FIFO_CNTL);

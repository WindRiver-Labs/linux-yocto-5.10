...
module_param(machinerestart, int, 0644);
 ...
 * Bit [15] = A write leveling error has occurred.
 *	     Error information can be found in the WRLVL_ERROR_STATUS parameter.
 * Bit [14] = A read leveling gate training error has occurred.
 *      Error information can be found in the RDLVL_ERROR_STATUS parameter.
...
#define SMMON_ATTR(_name, _event) \
	{ \
		.attr = __ATTR(_name, 0444, smmon_show, NULL), \
		.event = _event \
	}

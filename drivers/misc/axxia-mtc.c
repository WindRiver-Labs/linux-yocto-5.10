...
#define     NCP_AXIS_MTC_MTC_PRGM_MEM_START_ADDR(n)      \
	(0x00001000 + (4 * (n)))
...
#define   NCP_AXIS_MTC_MTC_TDO_CAPTURE_MEM_START_ADDR(n) \
	(0x00002000 + (4 * (n)))
 ...
 *  @details Registers to Control Initializing the TDO Capture and TESTPROG
 *  memories
 *  <table width="70%" align="center">
 *  <tr>
 *   <td colspan="4" align="center">
 ...
 *   <tr>
 * <td width="30%"> @param
 * curr_task_load_jtag_data_reg_continue_from_
 *     pause_dr_and_stop_in_pause_dr </td>
 *     <td width="20%" align="center"> 1 </td>
 ...
 *   <tr>
 *     <td width="30%"> @param
 * curr_task_load_jtag_inst_reg_continue_from_
 *     pause_ir_and_stop_in_pause_ir </td>
 *     <td width="20%" align="center"> 1 </td>
 *     <td width="20%" align="center"> 16 </td>
 *   <td width="30%"> Current task being processed is Load JTAG Instruction
 ...
 *        TASK definitions:Value:Description:TCK [29]:End-State
 ...
 *  @details DEBUG: MTC AXI Master External Program Memory Mode
 *  TDO Capture Monitor Debug Counters
 *  <table width="70%" align="center">
 *  <tr>
 *   <td colspan="4" align="center">
 *    struct ncp_axis_mtc_MTC_STATUS5_REG_ADDR_r_5600_t\n
...
mtc_dev_read(struct file *filp, char __user *data, size_t len, loff_t *ppose)
{
	...
	pr_debug("%s(%u @ %llu)\n", __func__, (unsigned int)len, *ppose);
	ptdo = dev->tdomem;
...
#ifdef DEBUG
	pr_debug("%s, tdo_bit=%d tdo_word=%d\n",
		 __func__, tdo_size_bit, tdo_size_word);
#endif
	      ...
	      loff_t *ppose)
{
	...
	/* Max 256 Words */
	mtc_buf = kmalloc(sizeof(u32) * 256, GFP_KERNEL);
	if (!mtc_buf)
		return -ENOMEM;
...
static long _mtc_config(struct mtc_device *dev,
			struct axxia_mtc_cfg_t *pmtccfg);
...
mtc_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case MTC_DEBUG_OP:
		...
		if (sizeof(struct mtc_regs) < (sizeof(u32) * (addr / 4))) {
			pr_debug("MTC Error: addr out of range\n");
			return -EFAULT;
		}
...
#ifdef DEBUG
			pr_debug("MTC_SINGLESTEP_ENABLE: dev->regs->config0=0x%x\n",
				 dev->regs->config0);
#endif
		}
	...
	case MTC_LOOPMODE_ENABLE:
		{
			...
			dev->regs->config0 = *((u32 *)&cfg0);
#ifdef DEBUG
			pr_debug("MTC_LOOPMODE_ENABLE dev->regs->config0=0x%x\n",
				 dev->regs->config0);
#endif
		}
...
#ifdef DEBUG
			pr_debug("MTC_AXI_EXT_PRGM_MEM_ENABLE axi_config0=0x%x\n",
				 dev->regs->axi_config0);
#endif
		} else {
			return -EPERM;
...
#ifdef DEBUG
			pr_debug("MTC_AXI_CAPT_WINDOW_PARAM_SET cfg0=0x%x cfg1=0x%x\n",
				 dev->regs->axi_config0,
				 dev->regs->axi_config1);
#endif
		} else {
	...
	case MTC_AXI_WATER_MARK_SET:
		...
		if (of_find_compatible_node(NULL, NULL, "axxia,axm5616") ||
			...
			if (wm.highwatermark > 0x3f || wm.lowwatermark > 0x3f)
				return -EINVAL;
...
#ifdef DEBUG
			pr_debug("MTC_AXI_MASTER_ADDR_SET cfg3=0x%x cfg4=0x%x\n",
				 dev->regs->axi_config3,
				 dev->regs->axi_config4);
